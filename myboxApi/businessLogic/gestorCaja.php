...
function guardarCajaNueva($datos)
{
    ...
    if (buscarNombreRepetido($datos->nombreCaja) == 0) {
        ...
        $destino = isset($datos->destino) && $datos->destino != '' ? $datos->destino : 'Buenos Aires';

        $id_caja = guardarCajaServicio($datos->nombreCaja, $datos->trackingCaja, count($listaTrackings), $destino, $datos->entrega, $datos->viaje, $datos->empresaUserId);
        ...
        if ($id_caja != 0) {
            ...
            if ($trackingsInvalidos == null) {
                //sendWppMessage("Se creo la caja " . $datos->nombreCaja . " que contiene " . count($listaTrackings) . " paquetes");
...
function consultarTrackingCajaDestinos($datos)
{

    $reponse = array();

    $listaTrackings = consultarTrackingsCajaServicio($datos->idCaja);
...
function paginacionCajaGestor($datos)
{
    ...
    $proc = "SELECT c.id,c.nombre,c.tracking,c.nPaquetes,DATE_FORMAT(c.fechaRegistro, '%d-%m-%Y') as 'Fecha Registro',c.fechaModificacion,c.estado,(SELECT SUM(p.peso) from paquete p WHERE p.cajaId = c.id) as 'pesoCaja',(SELECT SUM(p.total) from paquete p WHERE p.cajaId = c.id) as 'valorCaja',DATE_FORMAT(c.fechaEntrega, '%d-%m-%Y') as 'Fecha Entrega', DATE_FORMAT(c.fechaViaje, '%d-%m-%Y') as 'Fecha Viaje' from caja c WHERE estado = 1 ORDER BY c.nombre DESC LIMIT " . $offset . "," . $items;
...
function consultarCajasFranquicia($datos)
{
    return obtenerCajasFranquiciaServicio($datos->empresaId);
}
